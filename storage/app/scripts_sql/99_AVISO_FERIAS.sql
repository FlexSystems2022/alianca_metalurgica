...
WHERE SRH010.RH_DATAINI >= (SELECT DATEADD(dd,datediff(dd,0,DATEADD(DAY, 30, GETDATE())),0))
AND SRH010.RH_DATAINI <= (SELECT DATEADD(dd,datediff(dd,0,DATEADD(DAY, 30, GETDATE())),0))
...
AND NOT EXISTS(
	...
	AND Nexti.dbo.FLEX_AVISOS.NOME = 'Aviso de Férias'
)