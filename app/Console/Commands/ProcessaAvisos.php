...
class ProcessaAvisos extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ProcessaAvisos';
    ...
    {   
        $where = "  
            ...
            AND ".env('DB_OWNER')."FLEX_AVISOS.NOME = 'Aviso de Férias'
    ...
    private function enviaInformesNovos(){
        ...
        foreach ($informes as $informe) {
            ...
            $mensagem = 
            "Segue em anexo o seu aviso de férias.";
            ...
            if(file_exists($filePath)){
                ...
                var_dump("Não foi encontrado Aviso: " . $nomeInforme);
                $erro = "Não foi encontrado Aviso: " . $nomeInforme;
                $this->atualizaRegistroComErro($informe, 2, $erro);
                unlink($filePath);
                sleep(2);
            }
        }
        $this->info("Total de {$nInformes} aviso(s) informesNexti!");
    }

    private function enviaInformeNovo($informesNexti, $pdfInforme, $informe) {
        $name = $pdfInforme;
        $this->info("Enviando o aviso {$name} para API Nexti");
        ...
        if (!$this->restClient->isResponseStatusCode(201)) {
            ...
            $this->info("Problema ao enviar o aviso {$name} para API Nexti: {$errors}");
            $this->atualizaRegistroComErro($informe, 2, $errors);
        } else {
            $this->info("Aviso {$name} enviado com sucesso para API Nexti");
