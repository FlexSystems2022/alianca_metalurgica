...
class BuscaTrocaPostosAux extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'BuscaTrocaPostosAux';
    ...
    {
        //$dataRetro = date("Y-m-d", strtotime("-5 years"));
        $dataRetro = date("Y-m-d", strtotime("2020-11-21"));
    ...
    {
        $sql = "SELECT * FROM ".env('DB_OWNER')."NEXTI_RET_TROCA_POSTOS WHERE ".env('DB_OWNER')."NEXTI_RET_TROCA_POSTOS.ID = {$id}";
    ...
    private function addTrocaPosto($troca)
    {
        ...
        if ($trocaExistente) {
            $sql = "
                UPDATE ".env('DB_OWNER')."NEXTI_RET_TROCA_POSTOS
            ...
            $sql = "
                INSERT INTO ".env('DB_OWNER')."NEXTI_RET_TROCA_POSTOS
    ...
    {
        $dataInicio = date('Y-d-m H:i:s A');
        $this->restClient = new \Someline\Rest\RestClient('nexti');
        $this->restClient->withOAuthToken('client_credentials');
        $retBuscaTrocaPostos = $this->buscaTrocaPostos();
        $dataFim = date('Y-d-m H:i:s A');
        //$this->insertLogs("Trocas de postos foram buscadas do Nexti.", date("Y-m-d H:i:s A"), date("Y-m-d H:i:s A"));
    }
}
